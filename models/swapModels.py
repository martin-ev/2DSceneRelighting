...
class ImageEncoding(EncoderPart):
    def __init__(self, in_channels=3, enc_channels=32, out_channels=64):
        ...
        self.conv1 = nn.Sequential(
                        ...
                        nn.GroupNorm(1, enc_channels - in_channels),
                        nn.PReLU()
                     )
...
class Output(DecoderPart):
    def __init__(self, in_channels=64, out_channels=3):
        ...
        self.block = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, padding=1),  # should it be conv or transposed conv?
...
class SwapNet(nn.Module):
    def __init__(self, splitter, unsplitter):
